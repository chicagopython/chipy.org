...
from social_core.pipeline import (
    associate_by_email as super_associate_by_email)


def associate_by_email(*args, **kwargs):
    """Check if a user with this email already exists.
        If they do, don't create an account."""
    ...
    if email:
        ...
        if User.objects.filter(email=email).exists():
            msg = ugettext('This email is already in use. First login with '
                           'your other account and '
